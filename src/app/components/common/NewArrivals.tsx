      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {newProducts.map((product: Product) => (
            <Card 
              key={product.id} 
              className="group relative overflow-hidden bg-gradient-to-br from-purple-100 via-gray-100 to-purple-200 border border-green-500/20 hover:border-green-500/30 transition-all duration-300 hover:shadow-xl hover:shadow-green-500/20 cursor-pointer rounded-xl"
            >
              {/* Urgent Badge */}
              {product.urgent && (
                <div className="absolute top-3 left-3 z-10 bg-gradient-to-r from-red-500 to-red-600 text-white text-xs font-bold px-2 py-1 rounded-full shadow-lg animate-pulse">
              ...
              <CardHeader className="p-4 pb-2">
                {/* Category */}
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs text-green-800 font-semibold bg-green-500/20 px-2 py-1 rounded-full border border-green-500/30">
                    {product.category}
                  </span>
                  <span className={`text-xs font-medium px-2 py-1 rounded-full ${
                    product.stock === 'Limited Stock' 
                      ? 'text-red-700 bg-red-500/20 border border-red-500/30' 
                      : 'text-green-800 bg-green-500/20 border border-green-500/30'
                  }`}>
                    {product.stock}
                  </span>
                </div>
                ...
                <CardDescription className="text-sm text-gray-700 group-hover:text-gray-800 transition-colors duration-300 mt-2">
                  {product.description}
                </CardDescription>
              </CardHeader>
