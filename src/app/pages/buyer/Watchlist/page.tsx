...
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '../../../components/ui/card';
import { Button } from '../../../components/ui/button';
import { EyeOff, ShoppingCart, ExternalLink, Heart, Package, Star, CheckCircle } from 'lucide-react';
import BuyerSidebar from '../../../components/BuyerSidebar';
      ...
      <div className="flex-1 p-6 space-y-8">
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-gradient-to-br from-gray-900 via-gray-800 to-gray-900 mb-2 flex items-center">
              <Heart className="h-8 w-8 mr-3 text-green-400" />
              My Watchlist
            </h1>
            <p className="text-gray-800 text-base">Track and manage your favorite products</p>
          </div>
        ...
        <Card className="bg-gradient-to-br from-purple-100 via-gray-100 to-purple-200 border-slate-700 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300">
          <CardContent className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 text-center">
              <div>
                <div className="text-2xl font-bold text-gray-700">{watchlistItems.length}</div>
                <div className="text-sm text-gray-600">Total Items</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-green-600">{getInStockCount()}</div>
          ...
          {watchlistItems.length === 0 ? (
            <Card className="bg-gradient-to-br from-purple-100 via-gray-100 to-purple-200 border-slate-700 rounded-xl shadow-lg">
              <CardContent className="p-12 text-center">
                ...
                <p className="text-gray-700 mb-6">Start adding items to keep track of products you are interested in</p>
                <Button 
                  onClick={() => navigate('/search-inventory')}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {watchlistItems.map((item: WatchlistItem) => (
                <Card
                  key={item.id}
                  className="bg-gradient-to-br from-purple-100 via-gray-100 to-purple-200 border-slate-700 rounded-xl shadow-lg hover:shadow-xl hover:scale-[1.02] transition-all duration-300 overflow-hidden group"
                >
                  <div className="relative">
                    <img
                      src={item.imageUrl}
                    ...
                    />
                    <div className="absolute top-3 left-3">
                      <span className="bg-green-600/90 backdrop-blur-sm text-white px-2 py-1 rounded-md text-xs font-medium">
                        {item.category}
                      </span>
                    </div>
                    <div className="absolute top-3 right-3">
                  ...
                  <CardContent className="p-5 bg-white">
                    <div className="space-y-3">
                      ...
                      <div className="flex gap-3 pt-2">
                        <Button
                          className="flex-1 bg-green-600 hover:bg-green-700 text-white rounded-lg py-2.5 text-sm font-semibold transition-all duration-300 shadow-lg hover:shadow-xl"
                          onClick={() => handleViewDetails(item.id, item.inStock)}
                          disabled={!item.inStock}
                        >
                          <ExternalLink className="h-4 w-4 mr-2" />
                        ...
                        <Button
                          variant="outline"
                          size="icon"
                          className="border-red-500 text-red-600 hover:bg-red-50 hover:border-red-600 rounded-lg transition-all duration-300"
                          onClick={() => handleRemoveFromWatchlist(item.id)}
                          aria-label="Remove from watchlist"
                        >
                          <EyeOff className="h-4 w-4" />
