...
const HowItWorksPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-900">
      {/* Hero Section */}
      <section className="relative bg-gradient-to-r from-green-100 to-white py-8 sm:py-12 lg:py-16 xl:py-20 overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative text-center">
          <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold text-gray-700 leading-tight mb-4 sm:mb-7">
            How <span className="text-green-800">ZeeroStock</span> Works
          </h1>
          <p className="mt-2 sm:mt-4 text-base sm:text-lg lg:text-xl text-green-700 max-w-3xl mx-auto leading-relaxed mb-6 sm:mb-10 px-2">
            The leading B2B marketplace in India for industrial surplus, raw materials, and machinery. 
            Connect, trade, and grow your business with confidence.
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center space-y-3 sm:space-y-0 sm:space-x-4 mb-8 sm:mb-12 px-4">
            <button 
              className="w-full sm:w-auto px-6 sm:px-8 py-3 text-base sm:text-lg font-semibold rounded-lg shadow-lg shadow-green-500/20 hover:shadow-xl hover:shadow-green-500/30 transition-all duration-300 bg-gradient-to-r from-gray-600 to-gray-500 hover:from-green-700 hover:to-green-600 text-white border-0 flex items-center justify-center" 
              onClick={() => router.push('/pages/public/Login')}
            >
            ...
            <button 
              className="w-full sm:w-auto px-6 sm:px-8 py-3 text-base sm:text-lg font-semibold rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 border-2 border-gray-600 text-gray-500 hover:bg-green-400/10 bg-transparent hover:border-green-400/50" 
              onClick={() => router.push('/pages/public/Login')}
            >
              Get Started Free
            </button>
      ...
      {/* User Journey Section */}
      <div className="py-12 sm:py-16 lg:py-20 bg-gradient-to-r from-green-100 to-white min-h-screen">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-12 sm:mb-16">
            <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-gray-900 mb-3 sm:mb-4 px-2">Your Journey on ZeeroStock</h2>
            <p className="text-base sm:text-lg lg:text-xl text-gray-800 max-w-2xl mx-auto px-4">
              From registration to business growth - see how our platform transforms industrial trading
            </p>
          </div>
              ...
              <div key={step.id} className="relative">
                <Card className="h-full hover:shadow-xl transition-all duration-300 bg-gradient-to-br from-purple-100 via-gray-300 to-purple-200 backdrop-blur-sm border border-gray-700 hover:border-green-400/50 overflow-hidden">
                  ...
                  <CardHeader className="text-center pb-3 sm:pb-4 px-4 sm:px-6">
                    <div className={`bg-gradient-to-r ${step.color} w-12 h-12 sm:w-16 sm:h-16 rounded-full flex items-center justify-center text-white mx-auto mb-3 sm:mb-4 shadow-lg shadow-green-500/20`}>
                      {step.icon}
                    </div>
                    <div className="absolute -top-1 -left-1 bg-green-600 text-gray-900 w-6 h-6 sm:w-8 sm:h-8 rounded-full flex items-center justify-center font-bold text-xs sm:text-sm">
                      {step.id}
                    </div>
                    <CardTitle className="text-lg sm:text-xl font-semibold text-gray-900">{step.title}</CardTitle>
                  </CardHeader>
                  <CardContent className="text-center px-4 sm:px-6 pb-4 sm:pb-6">
                    <p className="text-gray-700 mb-3 sm:mb-4 text-sm sm:text-base">{step.description}</p>
                    <p className="text-xs sm:text-sm text-gray-500">{step.details}</p>
                  </CardContent>
                </Card>
                {index < userJourneySteps.length - 1 && (
                  <div className="hidden lg:block absolute top-1/2 -right-4 transform -translate-y-1/2 z-10">
                    <ArrowRight className="h-5 w-5 lg:h-6 lg:w-6 text-green-400" />
                  </div>
                )}
              </div>
            ))}
      ...
      {/* User Stories Section */}
      <div className="py-12 sm:py-16 lg:py-20 bg-gradient-to-r from-green-100 to-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-12 sm:mb-16">
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-gray-900 mt-3 sm:mt-5 mb-3 sm:mb-4 px-2">Real User Stories</h2>
            <p className="text-base sm:text-lg lg:text-xl text-gray-700 px-4">See how different users navigate and succeed on our platform</p>
          </div>

          {/* Story Type Selector */}
          <div className="flex justify-center mb-8 sm:mb-12 px-4">
            <div className="bg-gray-400/60 backdrop-blur-sm rounded-lg p-1 sm:p-2 shadow-lg border border-gray-700 w-full max-w-2xl">
              <div className="grid grid-cols-3 gap-1">
                {Object.entries(userStories).map(([key, story]: [string, UserStory]) => (
                  <Button
                    ...
                    variant={activeStory === key ? 'default' : 'ghost'}
                    onClick={() => setActiveStory(key as ActiveStoryType)}
                    className={`px-2 sm:px-6 py-2 sm:py-3 text-xs sm:text-sm font-medium ${activeStory === key ? 'bg-green-600 text-white hover:bg-green-700' : 'text-green-900 hover:bg-white'}`}
                  >
                    <span className="hidden sm:inline">{story.title.split(' ')[0]}</span>
                    <span className="sm:hidden">{story.title.split(' ')[0].slice(0, 6)}</span>
                  </Button>
                ))}
              </div>
            </div>
          ...
          {/* Story Content */}
          <div className="bg-gray-400/60 backdrop-blur-sm rounded-xl sm:rounded-2xl shadow-xl overflow-hidden border border-gray-700 mx-2 sm:mx-0">
            {/* Story Header */}
            <div className="bg-gradient-to-r from-green-400 to-green-500 text-white p-4 sm:p-6 lg:p-8">
              <div className="max-w-4xl mx-auto">
                <h3 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold mb-2 text-gray-700">{currentStory.title}</h3>
                <p className="text-lg sm:text-xl opacity-90 mb-3 sm:mb-4 text-gray-600">{currentStory.subtitle}</p>
                <div className="bg-white/10 rounded-lg p-3 sm:p-4 backdrop-blur-sm">
                  <p className="font-semibold mb-2 text-sm sm:text-base">{currentStory.persona}</p>
                  <p className="opacity-90 text-sm sm:text-base">{currentStory.scenario}</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-4 sm:p-6 lg:p-8">
              <div className="max-w-4xl mx-auto">
                <h4 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-gray-900 mb-6 sm:mb-8 text-center">The Journey</h4>
                <div className="space-y-6 sm:space-y-8">
                  {currentStory.journey.map((step: JourneyStep, index: number) => (
                    <div key={index} className="flex items-start gap-3 sm:gap-6">
                      <div className="flex-shrink-0">
                        <div className="bg-gradient-to-r from-green-500 to-green-600 w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center text-gray-700">
                          {step.icon}
                        </div>
                      </div>
                      <div className="flex-grow min-w-0">
                        <h5 className="text-lg sm:text-xl font-semibold text-gray-700 mb-2">{step.step}</h5>
                        <p className="text-gray-700 mb-2 text-sm sm:text-base break-words">{step.action}</p>
                        <p className="text-xs sm:text-sm text-black font-medium bg-green-400/10 p-2 sm:p-3 rounded-lg border border-green-400/20 break-words">
                          ✓ {step.outcome}
                        </p>
                      </div>
                    </div>
                ...
                {/* Results */}
                <div className="mt-8 sm:mt-12 bg-gradient-to-r from-green-400/10 to-green-600/10 rounded-xl p-4 sm:p-6 lg:p-8 border border-green-400/20">
                  <h5 className="text-lg sm:text-xl font-bold text-gray-900 mb-4 sm:mb-6 text-center">Results Achieved</h5>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
                    {Object.entries(currentStory.results).map(([key, value]: [string, string]) => (
                      <div key={key} className="text-center p-3 sm:p-0">
                        <div className="text-xl sm:text-2xl font-bold text-green-800 mb-2 break-words">{value}</div>
                        <div className="text-gray-900 capitalize text-sm sm:text-base">{key.replace('_', ' ')}</div>
                      </div>
                    ))}
                  </div>
                </div>
      ...
      <div className="py-12 sm:py-16 lg:py-20 bg-gradient-to-r from-green-100 to-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-12 sm:mb-16">
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-gray-900 mb-3 sm:mb-4 px-2">Key Features</h2>
            <p className="text-base sm:text-lg lg:text-xl text-gray-800 px-4">Powering smart manufacturing and efficient trading</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
            {platformFeatures.map((feature: PlatformFeature, index: number) => (
              <Card key={index} className="text-center hover:shadow-xl transition-all duration-300 bg-gradient-to-br from-purple-100 via-gray-300 backdrop-blur-sm border border-gray-700 hover:border-green-400/50 overflow-hidden">
                ...
                <CardHeader className="pb-3 sm:pb-4 px-4 sm:px-6">
                  <div className={`bg-gradient-to-r ${feature.color} w-12 h-12 sm:w-16 sm:h-16 rounded-xl flex items-center justify-center text-white mx-auto mb-3 sm:mb-4 shadow-lg shadow-green-500/20`}>
                    {feature.icon}
                  </div>
                  <CardTitle className="text-base sm:text-lg font-semibold text-gray-900">{feature.title}</CardTitle>
                </CardHeader>
      ...
      <section className="relative bg-gradient-to-r from-green-100 to-white py-12 sm:py-16 lg:py-20 overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative">
          <div className="mb-12 sm:mb-16">
            <h2 className="text-2xl sm:text-3xl lg:text-4xl xl:text-5xl font-extrabold text-gray-900 mb-3 sm:mb-4 px-2">
              Trusted by <span className="text-green-700">Industry Leaders</span>
            </h2>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-12 sm:mb-16">
            <div className="bg-gradient-to-br from-purple-100 via-gray-300 backdrop-blur-sm p-3 sm:p-6 rounded-xl border border-gray-700/50 hover:border-green-400/50 transition-all duration-300">
              <div className="text-xl sm:text-3xl font-bold text-green-800 mb-1 sm:mb-2">500+</div>
              <div className="text-xs sm:text-sm text-gray-900">Active Partners</div>
            </div>
            <div className="bg-gradient-to-br from-purple-100 via-gray-300 backdrop-blur-sm p-3 sm:p-6 rounded-xl border border-gray-700/50 hover:border-green-400/50 transition-all duration-300">
              <div className="text-xl sm:text-3xl font-bold text-green-800 mb-1 sm:mb-2">₹100M+</div>
              <div className="text-xs sm:text-sm text-gray-900">Inventory Moved</div>
            </div>
            <div className="bg-gradient-to-br from-purple-100 via-gray-300 backdrop-blur-sm p-3 sm:p-6 rounded-xl border border-gray-700/50 hover:border-green-400/50 transition-all duration-300">
              <div className="text-xl sm:text-3xl font-bold text-green-800 mb-1 sm:mb-2">15+</div>
              <div className="text-xs sm:text-sm text-gray-900">Industries Served</div>
            </div>
            <div className="bg-gradient-to-br from-purple-100 via-gray-300 backdrop-blur-sm p-3 sm:p-6 rounded-xl border border-gray-700/50 hover:border-green-400/50 transition-all duration-300">
              <div className="text-xl sm:text-3xl font-bold text-green-800 mb-1 sm:mb-2">99%</div>
              <div className="text-xs sm:text-sm text-gray-900">Success Rate</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6 lg:gap-8 items-center">
            {trustedCompanies.map((company: TrustedCompany, index: number) => (
              <div 
                key={index}
                className="group relative bg-gradient-to-br from-purple-100 via-gray-300 backdrop-blur-sm p-3 sm:p-4 lg:p-6 rounded-xl border border-gray-700/50 hover:border-green-400/50 transition-all duration-300 hover:shadow-2xl hover:shadow-green-500/20 cursor-pointer"
              >
                <div className="absolute top-1 right-1 sm:top-2 sm:right-2 w-2 h-2 sm:w-3 sm:h-3 bg-gray-600/20 rounded-full opacity-0 group-hover:opacity-100 transition-all duration-300 group-hover:animate-pulse"></div>
                <div className="absolute bottom-1 left-1 sm:bottom-2 sm:left-2 w-1.5 h-1.5 sm:w-2 sm:h-2 bg-gray-600/10 rounded-full opacity-0 group-hover:opacity-100 transition-all duration-500"></div>

                <div className="flex flex-col items-center space-y-2 sm:space-y-4">
                  <div className="w-10 h-10 sm:w-12 sm:h-12 lg:w-16 lg:h-16 bg-gradient-to-br from-gray-800/20 to-gray-900/20 rounded-xl flex items-center justify-center border border-gray-900/30 group-hover:scale-110 transition-transform duration-300">
                    <span className="text-black font-bold text-xs sm:text-sm lg:text-sm group-hover:text-green-100 transition-colors duration-300">
                      {company.logo}
                    </span>
                  </div>
                  
                  <div className="text-center">
                    <h3 className="text-gray-900 font-semibold text-xs sm:text-sm lg:text-sm group-hover:text-gray-900 transition-colors duration-300 mb-1">
                      {company.name}
                    </h3>
                    <p className="text-xs sm:text-xs lg:text-xs text-gray-700 group-hover:text-gray-900 transition-colors duration-300 break-words">
                      {company.industry}
                    </p>
                  </div>
                </div>
          ...
          <div className="mt-12 sm:mt-16 text-center">
            <p className="text-green-900 mb-4 sm:mb-6 text-base sm:text-lg px-4">
              Ready to join most trusted inventory marketplace of India?
            </p>
            <div className="flex flex-col sm:flex-row items-center justify-center space-y-3 sm:space-y-0 sm:space-x-4 px-4">
              <Button className="w-full sm:w-auto px-8 py-3 text-lg font-semibold rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 border-2 border-green-400/30 text-green-400 hover:bg-green-600/10 bg-transparent hover:border-green-400/50" onClick={() => router.push('/pages/public/Login')}>
                Become a Partner
              </Button>
              <Button variant="outline" className="w-full sm:w-auto px-8 py-3 text-lg font-semibold rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 border-2 border-green-400/30 text-green-400 hover:bg-green-600/10 bg-transparent hover:border-green-400/50" onClick={() => router.push('/pages/public/Login')}>
                View Case Studies
              </Button>
            </div>
          </div>
      ...
      {/* CTA Section */}
      <div className="py-12 sm:py-16 lg:py-20 bg-gradient-to-r from-green-100 to-white text-gray-900">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 text-center">
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold mb-4 sm:mb-6 px-2">Ready to Transform Your Industrial Trading?</h2>
          <p className="text-base sm:text-lg lg:text-xl mb-6 sm:mb-8 opacity-90 px-4">
            Join thousands of businesses already using ZeeroStock to buy and sell industrial products efficiently.
          </p>
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center px-4">
            <button className="w-full sm:w-auto px-6 sm:px-8 py-3 text-base sm:text-lg font-semibold rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 bg-white text-green-600 hover:bg-gray-100 hover:text-gray-900" onClick={() => router.push('/pages/public/Login')}>
              Start Buying Today
            </button>
            <button className="w-full sm:w-auto px-6 sm:px-8 py-3 text-base sm:text-lg font-semibold rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 bg-white text-green-600 hover:bg-gray-100 hover:text-gray-900" onClick={() => handleNavigation('/pages/public/Login')}>
              Become a Seller
            </button>
          </div>
        </div>
